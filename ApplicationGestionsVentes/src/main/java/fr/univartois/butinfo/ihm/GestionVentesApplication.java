...
import fr.univartois.butinfo.ihm.controller.CarnetClientsController;
import fr.univartois.butinfo.ihm.controller.ChoixSceneController;
import fr.univartois.butinfo.ihm.controller.FlotteVehiculeController;
import fr.univartois.butinfo.ihm.controller.StockArticlesController;
...
public class GestionVentesApplication extends Application {
    ...
    public void start(Stage stage) throws IOException {
        ...
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("./view/choix-scene-view.fxml"));
        Parent viewContent = fxmlLoader.load();

        // Ensuite, on la place dans une Scene...
		Scene scene = new Scene(viewContent, 800, 600);
        // que l'on place elle-même dans la fenêtre.
        stage.setScene(scene);

        // On peut ensuite donner un titre à la fenêtre.
        stage.setTitle("Flotte de véhicules");
        
     // On crée le modèle, et on le lie avec le contrôleur.
        ChoixSceneController controller = fxmlLoader.getController();
        Conducteurs conducteurs = Conducteurs.defaultConducteurs();
        FlotteVehicules flotte = FlotteVehicules.defaultFlotte();
        Stock stock = Stock.defaultStock();
        CarnetClients carnetClients = CarnetClients.defaultCarnetClients();
        flotte.setConducteurs(conducteurs);
        conducteurs.setFlotte(flotte);
        controller.setCarnetClients(carnetClients);
